...
public final class RegExpValidatorUtils {
	...
	public static String IsIDcard(String IDStr) {
        ...
        if (IDStr.length() == 18) {
        	String lastStr = IDStr.substring(17,18);
        	if ("X".equals(lastStr)) {
        		if (Ai.equals(IDStr.substring(0,17) + "x") == false) {
                    errorInfo = "身份证无效，不是合法的身份证号码";
                    return errorInfo;
                }
        	} else if (Ai.equals(IDStr) == false) {
                errorInfo = "身份证无效，不是合法的身份证号码";
                return errorInfo;
            }
        } else {
	...
	public static boolean IsMoney(String str) {
		String regex = "^(([1-9]{1}\\d{0,15})|([0]{1}))(\\.(\\d){0,2})?$";
		return str.matches(regex);
	}
